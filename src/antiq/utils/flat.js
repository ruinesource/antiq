...
export const flat = (doorOrStone, id) => {
  ...
  const item = get(
    normalizeId({ name: doorOrStone.name }, isDoor ? id : STONE_ID)
  )
...
const cloneFlat = (desc, level) => {
  ...
  if (Array.isArray(desc)) return level.map((item) => item && item.id)

  if (isPlainObject(desc)) {
    if (!level) return level

    let result = {}
    for (let key in level) result[key] = cloneFlat(desc[key], level[key])
