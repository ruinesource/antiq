...
export const userD = door(
  ...
  }),
  {
    ...
    // внутри методов есть асинхронность, поэтому actionId можем передавать только в аргументы
    // должен быть способ вызывать апи базу и ивенты изнутри ивентов других door
    // для этого в них должна быть возможность передавать actionId
    ...
    // при начале метода на бэк отправляется actionId и вся возможная информация
    ...
    // мы можем внутри каждого выполнения апи записывать в объект его функции actionId
    ...
    register: async ({ name, email }, opts) => {
      ...
      // нужен actionId, чтобы удалить юзера в случае ошибки внутри back или на outerApi
