...
export const userD = door(
  ...
  }),
  {
    ...
    // делаем очередь асинхронных операций, возвращаем результат бэка на фронт и продолжаем выполнять метод
    // в конце метода только success
    ...
    // Promise((resolve, reject) => ...promises; resolve(end); reject(innerPromisesSettedResults))
    // внутри детских промисов есть информация о родительских
    // с помощью этого можно записывать изменения (diff)
    // и отменять их при ошибке
    ...
    // зацепки между фронтом и бэком
    // это исключительно методы и апи
    // как же объединить выполнение метода и то что внутри?
