...
// если изменилось свойство
// а после ещё раз
// и первый раз ошибка
// то отмена отмены может затереть успех
// функция "вернуть состояние до ивента"
// должна оставлять все pending и уже записанные свойства
// для них, если они не изменятся, будет своя кнопка "вернуть состояние до ивента"
// если не хочется возврата состояния, нужно запомнить аргументы и переотправить событие
// например элемент появился в списке, и у него есть кнопка "переотправить"

// храним дату последнего сохранения каждого поля на сервере
// если при отмене ошибки поле было сохранено другой раз (перезаписано)
// то изменение не применяем
