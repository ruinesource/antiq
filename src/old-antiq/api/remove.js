...
export const remove = (normId) => {
  ...
  for (let parentNormId in parents) {
    ...
    const parentDiff = genParentDiff(
      parents[parentNormId],
      parent,
      normId,
      parent.id
    )
  ...
  g.iterationUpdates = {}
  g.prevItems = {}
  for (let childNormId in g.childs[normId] || {})
    delete g.graph[childNormId][normId]
  for (let parentNormId in g.graph[normId] || {})
