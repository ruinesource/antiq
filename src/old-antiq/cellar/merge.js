...
const merge = (desc, inst, diff, nextInst, parentNormId) => {
  ...
  if (isPlainObject(diff)) {
    ...
    if (isPlainObject(inst))
      for (let key in inst)
        if (!nextInst.hasOwnProperty(key)) nextInst[key] = inst[key]
    return nextInst
  }
  ...
  if (Array.isArray(diff)) {
    ...
    for (let i = 0; i < diff.length; i++) {
      ...
      if (inst && inst[i] && !prevChildPlaces[i]) {
        ...
        if (placePrevNormId)
          removeRelation(g.graph, placePrevNormId, parentNormId, g.stack)
      }
...
const genInst = (diff) =>
  Array.isArray(diff) ? [] : isPlainObject(diff) ? {} : diff
